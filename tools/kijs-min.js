...
parent=parent[part];}
return part;}
static defer(fn,millis,context,args){fn=this.createDelegate(fn,context,args);if(millis>0){return setTimeout(fn,millis);}else{fn();return 0;}}
...
parent=parent[part];}
return parent;}
static getText(key,variant='',args=null,languageId=null){if(kijs.isFunction(kijs.__getTextFn)){return kijs.__getTextFn.call(kijs.__getTextFnScope||this,key,variant,args,languageId);}
...
static isNumeric(value){return this.isNumber(value)||(this.isString(value)&&value.match(/^[0-9]+\.{0,1}[0-9]*$/));}
...
return ret;}};kijs.Data=class kijs_Data{constructor(config={}){this._columns=[];this._rows=[];this._primary=[];this._disableDuplicateCheck=false;this._configMap={disableDuplicateCheck:true,rows:true,columns:true,primary:true};if(kijs.isObject(config)){this.applyConfig(config);}}
...
return o;}
getRowsByFieldValue(columnName,value){const len=this._rows.length;const ret=[];for(let i=0,len;i<len;i++){if(this._rows[i][columnName]+''===value+''){ret.push(this._rows[i]);}}
return ret;}
...
const len=this._rows.length;for(let i=0,len;i<len;i++){let ok=true;for(let j=0;j<this._primary.length;j++){const col=this._primary[j];if(this._rows[i][col]+''!==keys[col]+''){ok=false;break;}}
...
return ret;}
static format(date,format){const _this=this;format=format+'';return format.replace(/[a-zA-Z]/g,function(letter){return _this.__formatReplace(letter,date);});}
...
return'';}};kijs.Number=class kijs_String{static format(number,decimals=0,decPoint='.',thousandsSep='\''){let ret='';let tmp=(number+'').split('.');if(decimals===''||decimals===null){if(tmp.length>1){decimals=tmp[1].length;}else{decimals=0;}}else{decimals=Number(decimals);}
if(!kijs.isEmpty(thousandsSep)&&!kijs.isEmpty(tmp[0])){const len=(tmp[0]+'').length;for(let i=len-1;i>=0;i--){ret=tmp[0].substr(i,1)+ret;if((len-i)%3===0&&i>0){ret=thousandsSep+ret;}}}else{ret=tmp[0]+'';}
if(decimals>0&&!kijs.isEmpty(ret)&&!kijs.isEmpty(decPoint)){let digits=tmp.length>1?tmp[1]+'':'';digits=kijs.String.padding(digits.substr(0,decimals),decimals,'0','right');ret+=decPoint+digits;}
return ret;}
...
static each(object,fn,context){for(let key in object){if(object.hasOwnProperty(key)){if(fn.call(context,key,object[key])===false){return;}}}}};kijs.Observable=class kijs_Observable{constructor(){this._events=this._events||{};}
...
on(names,callback,context){names=kijs.isArray(names)?names:[names];kijs.Array.each(names,function(name){if(!this.hasListener(name,callback,context)){if(!this._events[name]){this._events[name]=[];}
...
static htmlentities(html){return(html+'').replace(/[\u00A0-\u9999<>\&]/gim,function(i){return'&#'+i.charCodeAt(0)+';';});}
...
static padding(text,length,padString,type){length=length||0;text=text+'';while(text.length<length){if(type==='left'||type==='both'){text=padString+text;}
if(!type||type==='right'||type==='both'){text=text+padString;}}
return text;}
static repeat(text,multiplier){let response='';for(let i=0;i<multiplier;i++){response+=(text+'');}
return response;}
...
let ret='<ul>';kijs.Array.each(messages,function(msg){ret+='<li>'+msg+'</li>';},this);ret+='</ul>';return ret;}};kijs.gui.field=class kijs_gui_field{};kijs.gui.grid=class kijs_gui_grid{};kijs.gui.grid.cell=class kijs_gui_grid_cell{};kijs.gui.grid.columnConfig=class kijs_gui_grid_columnConfig{};kijs.gui.grid.filter=class kijs_gui_grid_filter{};kijs.UploadDialog=class kijs_UploadDialog extends kijs.Observable{constructor(config={}){super(false);this._uploadId=1;this._dropZones=[];this._contentTypes=[];this._currentUploadIds=[];this._uploadResponses={};this._validMediaTypes=['application','audio','example','image','message','model','multipart','text','video'];config=Object.assign({},{ajaxUrl:'index.php',multiple:true,directory:false,dropZones:[],filenameHeader:'X-Filename',pathnameHeader:'X-Filepath',maxFilesize:null},config);this._configMap={ajaxUrl:true,multiple:true,directory:true,filenameHeader:true,pathnameHeader:true,maxFilesize:true,dropZones:{target:'dropZone'},contentTypes:{target:'contentTypes'}};if(kijs.isObject(config)){this.applyConfig(config,true);}}
get contentTypes(){return this._contentTypes;}
set contentTypes(val){if(!kijs.isArray(val)){val=[val];}
kijs.Array.each(val,function(contentType){let parts=contentType.toLowerCase().split('/',2);if(!kijs.Array.contains(this._validMediaTypes,parts[0])){throw new kijs.Error('invalid media type "'+contentType+'"');}
...
set multiple(val){if(val===true&&this._browserSupportsDirectoryUpload()){this._multiple=true;}else{this._multiple=false;}}
...
_checkMime(filetype){if(filetype&&this._contentTypes.length>0){let parts=filetype.split('/',2);if(!kijs.Array.contains(this._contentTypes,parts[0])&&!kijs.Array.contains(this._contentTypes,parts.join('/'))){return false;}}
return true;}
_onDropZoneDrop(e){this._uploadFiles(e.nodeEvent.dataTransfer.files);}
_uploadFiles(fileList){this._uploadResponses={};if(fileList){for(let i=0;i<fileList.length;i++){if(this._checkMime(fileList[i].type)){this._uploadFile(fileList[i]);}else{this.raiseEvent('failUpload',this,fileList[i].name,fileList[i].type);}}}}
_uploadFile(file){let uploadId=this._uploadId++,headers={},filename=file.name,filedir=this._getRelativeDir(file.name,file.relativePath||file.webkitRelativePath),filetype=file.type||'application/octet-stream';headers[this._filenameHeader]=filename;headers[this._pathnameHeader]=filedir;headers['Content-Type']=filetype;kijs.Ajax.request({url:this._ajaxUrl,method:'POST',format:'json',headers:headers,postData:file,fn:this._onEndUpload,progressFn:this._onProgress,context:this,uploadId:uploadId});this._currentUploadIds.push(uploadId);this.raiseEvent('startUpload',this,filename,filedir,filetype,uploadId);}
...
if(response.errorMsg){let err=this._getMsg(response.errorMsg,'Fehler');kijs.gui.MsgBox.error(err.title,err.msg);if(response.errorMsg.cancelCb!==false){return;}}
if(response.warningMsg){let warn=this._getMsg(response.warningMsg,'Warnung');kijs.gui.MsgBox.warning(warn.title,warn.msg,function(e){if(e.btn==='ok'){this.do(facadeFn,data,fn,context,cancelRunningRpcs,waitMaskTarget,waitMaskTargetDomProperty,true);}},this);return;}
if(response.infoMsg){let info=this._getMsg(response.infoMsg,'Info');kijs.gui.MsgBox.info(info.title,info.msg);}
if(response.cornerTipMsg){let info=this._getMsg(response.cornerTipMsg,'Info');kijs.gui.CornerTipContainer.show(info.title,info.msg,'info');}
...
if(this._dom){this._dom.destruct();}
this._dom=null;this._target=null;super.destruct(true);}};kijs.gui.grid.columnConfig.ColumnConfig=class kijs_gui_grid_columnConfig_ColumnConfig extends kijs.Observable{constructor(config={}){super(false);if(config!==false){throw new kijs.Error('do not create a instance of kijs.gui.grid.columnConfig.ColumnConfig directly');}
...
resetDisplayAndOrder(){this._dom.style.order=0;this._dom.style.display='initial';}
setDisplayAndOrderByPattern(pattern,dataRowKey){if(pattern!==null&&pattern!==''){pattern=pattern+'';let value=(this._dataRow[dataRowKey]||'')+'';if(pattern.toLowerCase()===value.toLowerCase()){this._dom.style.order=-3;this.visible=true;}else if(pattern.length<=value.length&&pattern.toLowerCase()===value.toLowerCase().substr(0,pattern.length)){this._dom.style.order=-2;this.visible=true;}else if(pattern.length<=value.length&&value.toLowerCase().match(pattern.toLowerCase())){this._dom.style.order=-1;this.visible=true;}else{this._dom.style.order=1;this.visible=false;}}else{this._dom.style.order=0;this.visible=true;}}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
...
get isEmpty(){return kijs.isEmpty(this._dom.html)&&kijs.isEmpty(this._iconCls);}};kijs.gui.Mask=class kijs_gui_Mask extends kijs.gui.Element{constructor(config={}){super(false);this._iconEl=new kijs.gui.Icon({parent:this});this._targetX=null;this._targetDomProperty='dom';this._dom.clsAdd('kijs-mask');Object.assign(this._defaultConfig,{target:document.body});Object.assign(this._configMap,{displayWaitIcon:{target:'displayWaitIcon'},icon:{target:'icon'},iconChar:{target:'iconChar',context:this._iconEl},iconCls:{target:'iconCls',context:this._iconEl},iconColor:{target:'iconColor',context:this._iconEl},target:{target:'target'},targetDomProperty:true});if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
...
if(!superCall){this.raiseEvent('afterRender');}}
unrender(superCall){if(!superCall){this.raiseEvent('unrender');}
this._iconEl.unrender();super.unrender(true);}
...
return val;}
set value(val){val=val+'';val=val.split(this._separator);this._hour=val[0]?parseInt(val[0]):0;this._minute=val[1]?parseInt(val[1]):0;this._second=val[2]&&this._hasSeconds?parseInt(val[2]):0;this._clockMode=1;if(this._hour===24){this._hour=0;}
...
_zeroPad(number,lenght=2){number=number+'';while(number.length<lenght){number='0'+number;}
return number;}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._headerBar){this._headerBar.destruct();}
...
if(!hasPrimarys){return rows;}else if(!multiPrimarys){let ids=[],primaryKey=this._primaryKeys[0];kijs.Array.each(rows,function(row){ids.push(row.dataRow[primaryKey]);},this);return ids;}else{let ids=[];kijs.Array.each(rows,function(row){let idRow={};kijs.Array.each(this._primaryKeys,function(pk){idRow[pk]=row[pk];},this);ids.push(idRow);},this);return ids;}}
reload(){let selected=this.getSelectedIds();return this._remoteLoad(true).then((response)=>{if(selected){this.selectByIds(selected,false,true);}
return response;});}
rowsAdd(rows,startOffset=null){if(!kijs.isArray(rows)){rows=[rows];}
let renderStartOffset=this._rows.length,newRows=0,rowPos=0,offsets=[];kijs.Array.each(rows,function(row){let currentPos=null;if(row instanceof kijs.gui.grid.Row){row.parent=this;this._rows.push(row);currentPos=this._rows.length-1;}else{let pRow=this._getRowByPrimaryKey(row);if(pRow){pRow.updateDataRow(row);currentPos=this._rows.indexOf(pRow);}else{newRows++;this._rows.push(new kijs.gui.grid.Row({parent:this,dataRow:row,on:{click:this._onRowClick,context:this}}));currentPos=this._rows.length-1;}}
...
let showWaitMask=this.dom.node&&this.dom.node.parentNode&&(force||this._remoteDataLoaded===0);this._rpc.do(this._facadeFnLoad,args,function(response){this._remoteProcess(response,args,force);resolve(response);},this,true,showWaitMask?this:'none');}});}
...
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
kijs.Array.each(this._innerDomStack,function(dom){dom.destruct();},this);this._innerDomStack=null;super.destruct(true);}};kijs.gui.CornerTipContainer=class kijs_gui_CornerTipContainer extends kijs.gui.Container{constructor(config={}){super();this._dismissDelay=null;this._dom.clsRemove('kijs-container');this._dom.clsAdd('kijs-cornertipcontainer');Object.assign(this._defaultConfig,{dismissDelay:5000,width:230});Object.assign(this._configMap,{dismissDelay:true});if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
...
destruct(){super.destruct();}};kijs.gui.DataView=class kijs_gui_DataView extends kijs.gui.Container{constructor(config={}){super(false);this._currentEl=null;this._lastSelectedEl=null;this._data=[];this._facadeFnLoad=null;this._focusable=true;this._rpc=null;this._selectType='none';this._dom.clsRemove('kijs-container');this._dom.clsAdd('kijs-dataview');Object.assign(this._defaultConfig,{focusable:true,selectType:'single'});Object.assign(this._configMap,{autoLoad:{target:'autoLoad'},data:{target:'data'},disabled:{target:'disabled'},facadeFnLoad:true,focusable:{target:'focusable'},rpc:{target:'rpc'},selectType:true});if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}
this.applyConfig(config);this.on('keyDown',this._onKeyDown,this);this.on('elementClick',this._onElementClick,this);}
...
if(this._selectType==='none'){return null;}else if(this._selectType==='single'){return ret.length?ret[0]:null;}else{return ret;}}
load(args){this._rpc.do(this._facadeFnLoad,args,function(response){this.data=response.rows;if(!kijs.isEmpty(response.selectFilters)){this.selectByFilters(response.selectFilters);}
this.raiseEvent('afterLoad');},this,true,this,'dom',false);}
resetDisplayAndOrder(){kijs.Array.each(this._elements,function(el){el.resetDisplayAndOrder();},this);}
select(elements,keepExisting,preventSelectionChange){if(kijs.isEmpty(elements)){elements=[];}
if(!kijs.isArray(elements)){elements=[elements];}
if(!keepExisting){this.clearSelections(true);}
kijs.Array.each(elements,function(el){el.selected=true;},this);if(!preventSelectionChange){this.raiseEvent('selectionChange',{elements:elements,unSelect:false});}}
selectByFilters(filters,keepExisting,preventSelectionChange){if(kijs.isEmpty(filters)){filters=[];}
if(kijs.isObject(filters)){filters=[filters];}
for(let i=0;i<filters.length;i++){if(kijs.isObject(filters[i])){filters[i]=[filters[i]];}}
const selElements=[];if(!kijs.isEmpty(filters)){kijs.Array.each(this._elements,function(el){const row=el.dataRow;kijs.Array.each(filters,function(filterFields){let ok=false;kijs.Array.each(filterFields,function(filterField){if(kijs.isEmpty(filterField.value)||kijs.isEmpty(filterField.field)){throw new kijs.Error(`Unkown filter format.`);}
...
selectBetween(el1,el2,preventSelectionChange){let found=false;let elements=[];kijs.Array.each(this._elements,function(el){if(!found){if(el===el1){found='el1';}else if(el===el2){found='el2';}}
...
setDisplayAndOrderByPattern(pattern,dataRowKey){kijs.Array.each(this._elements,function(el){el.setDisplayAndOrderByPattern(pattern,dataRowKey);},this);}
unSelect(elements,preventSelectionChange){if(!kijs.isArray(elements)){elements=[elements];}
kijs.Array.each(elements,function(el){el.selected=false;},this);if(!preventSelectionChange){this.raiseEvent('selectionChange',{elements:elements,unSelect:true});}}
...
e.nodeEvent.preventDefault();break;case kijs.keys.SPACE:this._selectEl(this._currentEl,e.nodeEvent.shiftKey,e.nodeEvent.ctrlKey);e.nodeEvent.preventDefault();break;}}}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
...
set elements(val){if(!kijs.isArray(val)){val=[val];}
let elements=[];kijs.Array.each(val,function(element){if(kijs.isString(element)&&element==='-'){elements.push(new kijs.gui.Container({html:'<hr>'}));}else if(kijs.isString(element)){elements.push(new kijs.gui.Container({html:element}));}else{elements.push(element);}});this._spinbox.add(elements);}
...
close(){if(this._parentEl&&this._parentEl instanceof kijs.gui.Container&&this._parentEl.hasChild(this)){this._parentEl.remove(this);}
this.unrender();}
...
_onEnterPress(e){if(this._footerEl){kijs.Array.each(this._footerEl.elements,function(el){if(el instanceof kijs.gui.Button&&el.dom&&el.isDefault){el.raiseEvent('click');return;}},this);}}
...
get disabled(){return this._dom.clsHas('kijs-disabled');}
set disabled(val){if(val){this._dom.clsAdd('kijs-disabled');}else{this._dom.clsRemove('kijs-disabled');}
this._spinIconEl.disabled=val;this._errorIconEl.disabled=val;this._helpIconEl.disabled=val;if(this._spinBoxEl){this._spinBoxEl.disabled=val;}
...
validate(){this._errors=[];this._validationRules(this.value);this._displayErrors();return kijs.isEmpty(this._errors);}
...
_onSpinButtonClick(e){if(this.disabled){return;}
...
this._errors=null;this._labelDom=null;this._inputWrapperDom=null;this._spinBoxEl=null;this._spinIconEl=null;this._errorIconEl=null;this._helpIconEl=null;this._value=null;super.destruct(true);}};kijs.gui.grid.cell.Checkbox=class kijs_gui_grid_cell_Checkbox extends kijs.gui.grid.cell.Cell{constructor(config={}){super(false);this._checked=false;this._disabled=false;this._dom.clsAdd('kijs-grid-cell-checkbox');Object.assign(this._defaultConfig,{});Object.assign(this._configMap,{disabled:{target:'disabled'}});if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}
...
_getEditorArgs(){let eArgs=super._getEditorArgs();eArgs.hasTime=this._hasTime;eArgs.displayFormat=this._format;return eArgs;}
_setDomHtml(value){let date=kijs.Date.create(value);if(kijs.isDate(date)){this._dom.html=kijs.Date.format(date,this._format);}else{this._dom.html=value;}}};kijs.gui.grid.cell.Number=class kijs_gui_grid_cell_Number extends kijs.gui.grid.cell.Cell{constructor(config={}){super(false);this._numValue=null;this._editorXType='kijs.gui.field.Number';this._decimalPrecision=null;this._decimalPoint='.';this._decimalThousandSep='\'';this._numberStyles=[];this._unitBefore='';this._unitAfter='';Object.assign(this._defaultConfig,{cls:'kijs-grid-cell-number'});Object.assign(this._configMap,{decimalPrecision:true,decimalPoint:true,decimalThousandSep:true,numberStyles:{target:'numberStyles'},unitBefore:true,unitAfter:true});if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
...
super._applyToGrid();}
_getMenuButtons(){return kijs.Array.concat(this._getDefaultMenuButtons(),[{name:'btn_compare_checked',caption:kijs.getText('Alle angewählten'),iconChar:'&#xf096',on:{click:this._onFilterChange,context:this},style:{borderTop:'1px solid #aaaaaa'}},{caption:kijs.getText('Alle nicht angewählten'),name:'btn_compare_unchecked',iconChar:'&#xf096',on:{click:this._onFilterChange,context:this}}]);}
...
_getMenuButtons(){return kijs.Array.concat(this._getDefaultMenuButtons(),[{name:'btn_compare_begin',caption:kijs.getText('Feldanfang'),iconChar:'&#xf046',on:{click:this._onCompareBtnClick,context:this},style:{borderTop:'1px solid #aaaaaa'}},{caption:kijs.getText('Beliebiger Teil'),name:'btn_compare_part',iconChar:'&#xf096',on:{click:this._onCompareBtnClick,context:this}},{caption:kijs.getText('Ganzes Feld'),name:'btn_compare_full',iconChar:'&#xf096',on:{click:this._onCompareBtnClick,context:this}}]);}
...
render(superCall){super.render(true);this._searchField.renderTo(this._searchContainer.node);if(!superCall){this.raiseEvent('afterRender');}}};kijs.gui.FormPanel=class kijs_gui_FormPanel extends kijs.gui.Panel{constructor(config={}){super();this._data={};this._facadeFnLoad=null;this._facadeFnSave=null;this._fields=null;this._rpc=null;this._rpcArgs={};this._errorMsg=kijs.getText('Es wurden noch nicht alle Felder richtig ausgefüllt.');Object.assign(this._defaultConfig,{});Object.assign(this._configMap,{autoLoad:{target:'autoLoad'},data:{target:'data'},errorMsg:true,facadeFnLoad:true,facadeFnSave:true,rpc:{target:'rpc'},rpcArgs:true});this.on('add',this._observChilds,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
get autoLoad(){return this.hasListener('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}
set autoLoad(val){if(val){this.on('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}else{this.off('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}}
get data(){let data={};if(!kijs.isEmpty(this._fields)){kijs.Array.each(this._fields,function(field){if(field.submitValue!==false){data[field.name]=field.value;}else{delete this._data[field.name];}},this);}
...
for(let i=0;i<this._fields.length;i++){this._fields[i].value=null;}
...
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
this._value=null;super.destruct(true);}};kijs.gui.Window=class kijs_gui_Window extends kijs.gui.Panel{constructor(config={}){super();this._resizeDeferHandle=null;this._dragInitialPos=null;this._modalMaskEl=null;this._draggable=false;this._resizeDelay=300;this._targetX=null;this._targetDomProperty='dom';this._dom.clsAdd('kijs-window');Object.assign(this._defaultConfig,{draggable:true,target:document.body,closable:true,maximizable:true,resizable:true,shadow:true});Object.assign(this._configMap,{draggable:{target:'draggable'},modal:{target:'modal'},resizeDelay:true,target:{target:'target'},targetDomProperty:true});this.on('mouseDown',this._onMouseDown,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
...
set value(val){if(val===this._valueUnchecked||val===false||val===0||val==='0'){this._checked=0;}else if(val===this._valueChecked||val===true||val===1||val==='1'){this._checked=1;}else if(val===this._valueDeterminated||val===2){this._checked=2;}else{throw new kijs.Error(`config"value"is not valid.`);}
...
if(this._iconEl){this._iconEl.destruct();}
if(this._captionDom){this._captionDom.destruct();}
this._checkboxIconEl=null;this._iconEl=null;this._captionDom=null;super.destruct(true);}};kijs.gui.field.Combo=class kijs_gui_field_Combo extends kijs.gui.field.Field{constructor(config={}){super(false);this._minSelectCount=null;this._maxSelectCount=null;this._caption=null;this._oldCaption=null;this._oldValue=null;this._value='';this._keyUpDefer=null;this._serverSort=false;this._inputDom=new kijs.gui.Dom({disableEscBubbeling:true,nodeTagName:'input',nodeAttribute:{id:this._inputId}});this._listViewEl=new kijs.gui.ListView({cls:'kijs-field-combo',autoLoad:false,focusable:false});this._spinBoxEl=new kijs.gui.SpinBox({target:this,targetDomProperty:'inputWrapperDom',ownerNodes:[this._inputWrapperDom,this._spinIconEl.dom],openOnInput:true,elements:[this._listViewEl],style:{maxHeight:'400px'}});this._dom.clsAdd('kijs-field-combo');Object.assign(this._defaultConfig,{spinIconVisible:true,minChars:'auto'});Object.assign(this._configMap,{autoLoad:{target:'autoLoad'},serverSort:true,showCheckBoxes:{target:'showCheckBoxes',context:this._listViewEl},selectType:{target:'selectType',context:this._listViewEl},facadeFnLoad:{target:'facadeFnLoad',context:this._listViewEl},rpc:{target:'rpc',context:this._listViewEl},minChars:{target:'minChars',prio:2},captionField:{target:'captionField',context:this._listViewEl},iconCharField:{target:'iconCharField',context:this._listViewEl},iconClsField:{target:'iconClsField',context:this._listViewEl},iconColorField:{target:'iconColorField',context:this._listViewEl},toolTipField:{target:'toolTipField',context:this._listViewEl},valueField:{target:'valueField',context:this._listViewEl},minSelectCount:true,maxSelectCount:true,data:{prio:1000,target:'data',context:this._listViewEl},value:{prio:1001,target:'value'}});this._eventForwardsAdd('input',this._inputDom);this._eventForwardsAdd('blur',this._inputDom);this._eventForwardsAdd('keyDown',this._inputDom);this._inputDom.on('keyUp',this._onInputKeyUp,this);this._inputDom.on('keyDown',this._onInputKeyDown,this);this._inputDom.on('change',this._onInputChange,this);this._spinBoxEl.on('click',this._onSpinBoxClick,this);this._listViewEl.on('click',this._onListViewClick,this);this._listViewEl.on('afterLoad',this._onListViewAfterLoad,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
get autoLoad(){return this.hasListener('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}
set autoLoad(val){if(val){this.on('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}else{this.off('afterFirstRenderTo',this._onAfterFirstRenderTo,this);}}
...
set disabled(val){super.disabled=!!val;if(val||this._dom.clsHas('kijs-readonly')){this._inputDom.nodeAttributeSet('readOnly',true);}else{this._inputDom.nodeAttributeSet('readOnly',false);}
...
set readOnly(val){super.readOnly=!!val;this._listViewEl.disabled=val||this._dom.clsHas('kijs-disabled');if(val||this._dom.clsHas('kijs-disabled')){this._inputDom.nodeAttributeSet('readOnly',true);}else{this._inputDom.nodeAttributeSet('readOnly',false);}}
get rpc(){return this._listViewEl.rpc;}
set rpc(val){this._listViewEl.rpc=val;}
get value(){return this._value;}
set value(val){this._oldcaption=this._caption;this._oldValue=this._value;this._caption=this._getCaptionFromValue(val);this._value=val;this._listViewEl.value=val;this._inputDom.nodeAttributeSet('value',this._caption);}
load(args){args=args?args:{};if(this._serverSort){let input=this._inputDom.nodeAttributeGet('value');args.input=kijs.isString(input)?input:'';args.value=this._value;if(args.input.length>=this._minChars||this._value!==''){this._listViewEl.load(args);}}else{this._listViewEl.load(args);}}
...
_getCaptionFromValue(val){let found=false;let caption=null;kijs.Array.each(this._listViewEl.data,function(row){if(row[this._listViewEl.valueField]===val){found=true;caption=row[this._listViewEl.captionField];return false;}},this);if(!found){caption=val;}
return caption;}
_setProposal(key){let inputVal=this._inputDom.nodeAttributeGet('value'),matchVal='';inputVal=(inputVal+'').trim();if(inputVal&&key!=='Backspace'&&key!=='Delete'){kijs.Array.each(this._listViewEl.data,function(row){if(kijs.isString(row[this._listViewEl.captionField])&&row[this._listViewEl.captionField].toLowerCase()===inputVal.toLowerCase()){matchVal=row[this._listViewEl.captionField];return false;}},this);if(matchVal===''){kijs.Array.each(this._listViewEl.data,function(row){let caption=row[this._listViewEl.captionField];if(kijs.isString(row[this._listViewEl.captionField])&&inputVal.length<=caption.length&&caption.substr(0,inputVal.length).toLowerCase()===inputVal.toLowerCase()){matchVal=row[this._listViewEl.captionField];return false;}},this);}
...
this._listViewEl.setDisplayAndOrderByPattern(inputVal,this.captionField);}else if(key==='Backspace'||key==='Delete'){this._listViewEl.setDisplayAndOrderByPattern(inputVal,this.captionField);}else{this._listViewEl.resetDisplayAndOrder();}}
_validationRules(value){if(this._required){if(kijs.isEmpty(value)){this._errors.push('Dieses Feld darf nicht leer sein');}}
if(!kijs.isEmpty(this._minSelectCount)){const minSelectCount=this._minSelectCount;if(kijs.isArray(value)){if(value.length<minSelectCount){this._errors.push(`Min.${minSelectCount}müssen ausgewählt werden`);}}else if(kijs.isEmpty(value)&&minSelectCount>0){this._errors.push(`Min.${minSelectCount}müssen ausgewählt werden`);}}
...
_onAfterFirstRenderTo(e){this.load();}
_onInputKeyDown(e){this._listViewEl._onKeyDown(e);if(e.nodeEvent.key==='Enter'){let dataViewElement=this._listViewEl.getSelected();this._spinBoxEl.close();if(dataViewElement){this.value=dataViewElement.dataRow[this._listViewEl.captionField];}}}
_onInputKeyUp(e){if(kijs.Array.contains(['ArrowDown','ArrowUp','ArrowLeft','ArrowRight','Delete','Insert','Home','End','Alt','AltGraph','ContextMenu','Control','Shift','Enter','CapsLock','Tab','OS','Escape'],e.nodeEvent.key)){return;}
if(this._keyUpDefer){window.clearTimeout(this._keyUpDefer);this._keyUpDefer=null;}
this._keyUpDefer=kijs.defer(function(){if(this._serverSort){this.load();}else{this._setProposal(e.nodeEvent.key);}},500,this);}
_onInputChange(e){let inputVal=this._inputDom.nodeAttributeGet('value'),matchVal='';inputVal=(inputVal+'').trim();kijs.Array.each(this._listViewEl.data,function(row){if(kijs.isString(row[this._listViewEl.captionField])&&row[this._listViewEl.captionField].toLowerCase()===inputVal.toLowerCase()){matchVal=row[this._listViewEl.captionField];return false;}},this);if(matchVal&&matchVal!==this.value){this.value=matchVal;this.raiseEvent('change');}else if(!matchVal){this.value=this._value;}}
_onListViewAfterLoad(e){this.value=this._value;}
_onListViewClick(e){this._spinBoxEl.close();this.value=this._listViewEl.value;this.raiseEvent('change');}
_onSpinBoxClick(){this._inputDom.focus();}
_onSpinButtonClick(e){super._onSpinButtonClick(e);this._listViewEl.resetDisplayAndOrder();}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._inputDom){this._inputDom.destruct();}
this._inputDom=null;this._listViewEl=null;this._oldValue=null;super.destruct(true);}};kijs.gui.field.DateTime=class kijs_gui_field_DateTime extends kijs.gui.field.Field{constructor(config={}){super(false);this._hasTime=true;this._hasDate=true;this._hasSeconds=false;this._timeRequired=false;let useDefaultSpinIcon=!kijs.isDefined(config.spinIconChar);this._inputDom=new kijs.gui.Dom({disableEscBubbeling:true,nodeTagName:'input',nodeAttribute:{id:this._inputId},on:{change:this._onChange,context:this}});this._dom.clsAdd('kijs-field-datetime');this._timePicker=new kijs.gui.TimePicker({on:{change:this._onTimePickerChange,afterRender:this._onTimePickerAfterRender,context:this}});this._spBxSeparator=new kijs.gui.Element({cls:'kijs-separator'});this._datePicker=new kijs.gui.DatePicker({on:{dateSelected:this._onDatePickerSelected,dateChanged:this._onDatePickerChange,context:this}});this._spinBoxEl=new kijs.gui.SpinBox({target:this,width:383,height:260,cls:['kijs-flexrow','kijs-spinbox-datetime'],targetDomProperty:'inputWrapperDom',ownerNodes:[this._inputWrapperDom,this._spinIconEl.dom],openOnInput:false,parent:this,elements:[this._datePicker,this._spBxSeparator,this._timePicker],on:{show:this._onSpinBoxShow,context:this}});Object.assign(this._defaultConfig,{spinIconVisible:true,spinIconChar:'&#xf073',displayFormat:'d.m.Y H:i:s',valueFormat:'Y-m-d H:i:s'});Object.assign(this._configMap,{hasTime:true,hasDate:true,hasSeconds:true,timeRequired:true,displayFormat:true,valueFormat:true});this._eventForwardsAdd('blur',this._inputDom);this._eventForwardsAdd('change',this._inputDom);this._eventForwardsAdd('input',this._inputDom);this._eventForwardsRemove('enterPress',this._dom);this._eventForwardsRemove('enterEscPress',this._dom);this._eventForwardsRemove('escPress',this._dom);this._eventForwardsAdd('enterPress',this._inputDom);this._eventForwardsAdd('enterEscPress',this._inputDom);this._eventForwardsAdd('escPress',this._inputDom);this.on('input',this._onInput,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}
...
return format;}
_getDateTimeByString(dateTimeStr){let year=null,month=null,day=null,hour=0,minute=0,second=0,timeMatch=false,dateTimeAr,timeStr,dateStr;dateTimeStr=dateTimeStr+'';if(dateTimeStr.includes(" ")&&this._hasDate){dateTimeAr=dateTimeStr.split(" ");dateStr=dateTimeAr[0];if(dateTimeAr.length>1){kijs.Array.each(dateTimeAr,function(item,i){if(i>0){timeStr=timeStr+dateTimeAr[i];}});}else{timeStr=dateTimeAr[1];}}else{timeStr=dateTimeStr;dateStr=dateTimeStr;}
...
_onTimePickerChange(e){let v=this._getDateTimeByString(this.value)||new Date();this.value=this._format('Y-m-d',v)+' '+e.value;this._spinBoxEl.close();this.validate();this.raiseEvent('change',{value:this.value});}
...
this.validate();this.raiseEvent('change',{value:this.value});}
...
_getLinkType(value){value=value+'';if(value.match(/^\s*\+?[0-9\s]+$/i)){return'tel';}
if(value.match(/^[^@]+@[^@\.]+\.[a-z]+$/i)){return'mail';}
if(value.match(/^[\w0-9-öüäéèà\.]+\.[a-z]{2,}$/i)){return'web';}
return false;}
_openLink(link,type){if(type==='tel'){window.open('tel:'+link.replace(/[^\+0-9]/i,''));}else if(type==='mail'){window.open('mailto:'+link,'');}else if(type==='web'&&link.substr(0,4)==='http'){window.open(link,'');}else if(type==='web'){window.open('http://'+link,'');}}
_onDomClick(){if(this._link){let linkType=this._linkType==='auto'?this._getLinkType(this.value):this._linkType;this._openLink(this.value+'',linkType);}}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._inputDom){this._inputDom.destruct();}
...
render(superCall){super.render(true);if(!this._aceEditor){this._aceEditorNode=document.createElement('div');this._inputWrapperDom.node.appendChild(this._aceEditorNode);this._aceEditor=ace.edit(this._aceEditorNode);this._inputNode=this._aceEditorNode.firstChild;this._inputNode.id=this._inputId;kijs.defer(function(){var _this=this;this._aceEditor.getSession().on('change',function(){_this.raiseEvent(['input','change']);});},200,this);}
...
if(!kijs.isEmpty(this._maxSelectCount)){const maxSelectCount=this._maxSelectCount;if(kijs.isArray(value)){if(value.length>maxSelectCount){this._errors.push(`Max.${maxSelectCount}dürfen ausgewählt werden`);}}else if(!kijs.isEmpty(value)&&maxSelectCount<1){this._errors.push(`Max.${maxSelectCount}dürfen ausgewählt werden`);}}}
_onAfterFirstRenderTo(e){this.load();}
_onListViewSelectionChange(e){const val=this.value;this._value=val;this.raiseEvent(['input','change'],{oldValue:this._oldValue,value:val});this._oldValue=val;this.validate();}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._listView){this._listView.destruct();}
this._listView=null;this._oldValue=null;super.destruct(true);}};kijs.gui.field.Memo=class kijs_gui_field_Memo extends kijs.gui.field.Field{constructor(config={}){super(false);this._inputDom=new kijs.gui.Dom({disableEnterBubbeling:true,disableEscBubbeling:true,nodeTagName:'textarea',nodeAttribute:{id:this._inputId}});this._trimValue=true;this._dom.clsAdd('kijs-field-memo');Object.assign(this._configMap,{trimValue:true});this._eventForwardsAdd('input',this._inputDom);this._eventForwardsAdd('change',this._inputDom);this._eventForwardsAdd('blur',this._inputDom);this._eventForwardsRemove('enterPress',this._dom);this._eventForwardsRemove('enterEscPress',this._dom);this._eventForwardsRemove('escPress',this._dom);this._eventForwardsAdd('enterPress',this._inputDom);this._eventForwardsAdd('enterEscPress',this._inputDom);this._eventForwardsAdd('escPress',this._inputDom);this.on('input',this._onInput,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
get disabled(){return super.disabled;}
...
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._inputDom){this._inputDom.destruct();}
this._inputDom=null;super.destruct(true);}};kijs.gui.field.Password=class kijs_gui_field_Password extends kijs.gui.field.Field{constructor(config={}){super(false);this._inputDom=new kijs.gui.Dom({disableEscBubbeling:true,nodeTagName:'input',nodeAttribute:{id:this._inputId,type:'password'}});this._disableBrowserSecurityWarning=false;this._passwordChar='•';this._trimValue=false;this._value=null;this._dom.clsAdd('kijs-field-password');Object.assign(this._defaultConfig,{disableBrowserSecurityWarning:'auto'});Object.assign(this._configMap,{disableBrowserSecurityWarning:{prio:-1,target:'disableBrowserSecurityWarning'},passwordChar:true,trimValue:true});this._eventForwardsAdd('blur',this._inputDom);this._eventForwardsAdd('change',this._inputDom);this._eventForwardsRemove('enterPress',this._dom);this._eventForwardsRemove('enterEscPress',this._dom);this._eventForwardsRemove('escPress',this._dom);this._eventForwardsAdd('enterPress',this._inputDom);this._eventForwardsAdd('enterEscPress',this._inputDom);this._eventForwardsAdd('escPress',this._inputDom);this._inputDom.on('input',this._onDomInput,this);this.on('input',this._onInput,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
...
if(val){this._inputDom.nodeAttributeSet('type',undefined);this._inputDom.on('keyUp',this._onKeyUp,this);this._inputDom.on('mouseUp',this._onMouseUp,this);this._inputDom.on('input',this._onInput,this);}else{this._inputDom.nodeAttributeSet('type','password');this._inputDom.off('keyUp',this._onKeyUp,this);this._inputDom.off('mouseUp',this._onMouseUp,this);this._inputDom.off('input',this._onInput,this);}
...
_validationRules(originalValue){super._validationRules(originalValue);let value=originalValue+'';if(value.trim()!==''){if(this._thousandsSeparator!==''){value=kijs.String.replaceAll(value,this._thousandsSeparator,'');}
