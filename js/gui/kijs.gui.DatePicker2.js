...
// kijs.gui.DatePicker
// --------------------------------------------------------------
kijs.gui.DatePicker2 = class kijs_gui_DatePicker2 extends kijs.gui.Element {
    ...
    constructor(config={}) {
        ...
        this._mode = 'day';             // Modus: 'day', 'week' oder 'range'
        
        this._date = null;              // Aktueller Wert (Bei mode 'week' und 'range' ist es das Startdatum)
        this._endDate = null;           // Enddatum des aktuellen Werts (nur bei Mode 'week' und 'range', sonst null
        ...
        this._calendarDivDom = new kijs.gui.Dom({ cls: 'kijs-datepicker2-calendardiv' });
        this._calendarWeekHeadersDivDom = new kijs.gui.Dom({ cls: 'kijs-datepicker2-weakheadersdiv' });
        
        this._footerDivDom = new kijs.gui.Dom({ cls: 'kijs-datepicker2-footerdiv' });
        ...
        this._dom.clsAdd('kijs-datepicker2');
        ...
        Object.assign(this._configMap, {
            mode: true,                         // Modus: 'day', 'week' oder 'range'
            weekNumbersHide: true,              // Wochennummern ausblenden?
            startWeekday: true,                 // Erster Tag in der Ansicht (1=Montag)
            ...
            endValue: { target: 'endValue' }    // (Date Object oder SQL-String mit einem beliebigen Datum des Monats)
        });
    ...
    get date() {
        if (kijs.isEmpty(this._date)) {
            ...
            if (!kijs.isEmpty(this._endDate) && this._endDate < this._date) {
                return this._endDate;
            } else {
                return this._date;
            }
    ...
    get endDate() {
        ...
        // Sicherstellen, dass das Enddatum hinter dem Startdatum liegt
        } else if (this._endDate < this._date) {
            return this._date;
    ...
    set endDate(val) {
        if (kijs.isEmpty(val)) {
            ...
            this._endDate = kijs.Date.getDatePart(kijs.Date.create(val));
        }
        this._calculate();
    }
    ...
    get endValue() {
        if (kijs.isEmpty(this._endDate)) {
            ...
            return kijs.Date.format(this._endDate, this._valueFormat);
        }
    }
    set endValue(val) { 
        this.endDate = val;
    ...
    get startWeekday() { return this._startWeekday; }
    set startWeekday(val) { this._startWeekday = parseInt(val); }
    ...
    _checkValueOrder() {
        if (this._mode === 'day') {
            ...
            if (!kijs.isEmpty(this._date)) {
                ...
                // Enddatum
                this._endDate = kijs.Date.addDays(this._date, 6);
            }
    ...
    _onTodayBtnClick(e) {
        ...
        this._date = this.getNextValidDate(today);
        if (this._mode === 'range') {
            this._endDate = kijs.Date.clone(this._date);
        } else {
            this._endDate = null;
        }
        this._monthPicker.date = this._date;
        ...
        if (!kijs.Date.compare(this._date, today)) {
    ...
    _onItemDomClick(e) {
        if (e.dom.clsHas('kijs-day') && !e.dom.clsHas('kijs-disabled')) {
            const curDate = kijs.isEmpty(this._date) ? null : kijs.Date.clone(this._date);
            const curEndDate = kijs.isEmpty(this._endDate) ? null : kijs.Date.clone(this._endDate);
            ...
            if (kijs.Date.compare(date, e.dom.date)) {
                ...
                if (this._mode === 'range' && !kijs.isEmpty(this._date) && kijs.isEmpty(this._endDate)) {
                    this._endDate = date;
